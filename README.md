...
一个基于 **WXT + Vue 3 + Tiptap** 的 Chrome 侧边栏（Side Panel）笔记应用：点击扩展图标即可在浏览器侧边栏中快速记笔记、整理文件夹、全文搜索，数据全部存储在本地（`chrome.storage.local`），无需服务器、离线可用。
...
私有项目（`package.json` 中 `private: true`），仅供个人/团队内部使用。
